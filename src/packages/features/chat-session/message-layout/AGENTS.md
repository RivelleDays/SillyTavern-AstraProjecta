...
- Reasoning target: native `.mes_reasoning_header` remains the SillyTavern toggle target. Astra inserts only `.astra-mesReasoningSparkle` before `.mes_reasoning_header_title` and `.astra-mesReasoningChevron` after it when present, and leaves native `.mes_reasoning_details`, `.mes_reasoning_summary`, and `.mes_reasoning` in place.
