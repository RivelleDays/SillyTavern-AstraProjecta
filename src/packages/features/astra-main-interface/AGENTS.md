...
- Treat favorite scope values such as `favorite:character:<id>` and `favorite:group:<id>` as Astra body-routing state only. Selecting one must not directly switch SillyTavern's active chat unless a later feature explicitly owns native activation.
