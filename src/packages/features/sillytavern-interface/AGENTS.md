...
- Keep SillyTavern tool route descriptors, live host surfaces, route navigation, route icon implementation, and SillyTavern interface CSS together as one feature package.

## Owned Paths / Responsibilities

- Portable route and route-icon key contracts live in `src/app/shared/sillytavern-interface/`; this feature consumes those contracts for implementation.
- `contracts/` owns stable SillyTavern interface panel DOM ids, native host ids, and storage keys. Send-form trigger ids do not live here.
...
- `icons/` owns route SVG assets and raw SVG loading contracts used by the mobile assembly adapter.
...
- Do not import from `src/packages/features/chat-session/send-form`; `app/mobile` may adapt this feature for send-form, but this package must stay independent from send-form UI.
