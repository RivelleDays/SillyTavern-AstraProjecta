...
- Exposes an activation-only character/group action for Favorite scope buttons. Activation-only calls must not set a target chat id: they activate the entity and allow SillyTavern to load its remembered `character.chat` or `group.chat_id`, then verify the resulting context before reporting success.
...
- Prefer public `selectCharacterById(..., { switchMenu: false })` for activation-only character switches, with the existing native character selector as an availability fallback. Activation-only group switches may use the existing native group selector and `/go` fallback because the public context does not expose `openGroupById`.
