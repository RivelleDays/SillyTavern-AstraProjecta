---
name: Bug report
about: Report a reproducible issue with AstraProjecta
title: "[Bug] "
labels: bug
assignees: ''

---

## Self-check

Please confirm these before submitting:

- [ ] I am using the latest SillyTavern staging branch.
- [ ] I disabled all non-built-in extensions.
- [ ] I disabled custom CSS and custom regex.
- [ ] I tested the issue in Chrome.
- [ ] I can reproduce the issue reliably at least twice.

---

## Version / Build Info

- AstraProjecta version:
- SillyTavern version and branch:
- AstraProjecta last update date:
- Anything changed recently? <!-- yes/no + what changed -->

---

## System Info

- Device: <!-- PC / Mac / iPhone model / Android phone / etc. -->
- OS: <!-- Windows 11 / macOS 15 / iOS 18 / Android 15 / etc. -->
- Browser: <!-- Chrome version; include other browsers if tested -->
- Screen size: <!-- optional, e.g. 1920×1080 / 390px mobile width -->

---

## What happened?

<!-- Give a clear and concise description of the issue. -->

---

## Steps to reproduce

<!-- Step-by-step reproduction is strongly recommended. -->

1.
2.
3.

---

## Expected result

<!-- What did you expect to happen? -->

---

## Actual result

<!-- What actually happened? Include error messages if any. -->

---

## Screenshots / Recording

<!-- Strongly recommended. Attach images or a short screen recording. Highlight the relevant area if possible. -->

---

## Extra context

<!-- Optional, but helpful:
- Console errors or logs
- Related AstraProjecta settings
- Mobile vs desktop behavior
- Whether it happens in a new or clean chat
- Whether it happens with a specific character, group, or chat
-->
